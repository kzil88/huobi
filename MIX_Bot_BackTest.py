...
if __name__ == '__main__':
    ...
    for i in range(200,len(date_seq)):
        ...
        for j in range(len(coin_list)):
            ...
            ans = singal2(coin_list[j],date_seq[i-200:i],para_min,para_resu[0],para_resu[1])
            # ans2 是预警信号，留待后用
            ...
            if ans == 1 and ans2 == 0 and can_buy[j] == 1:
                ...
                op.sell(coin_list[j],date_seq[i-1],-1,para_min)
            lock = op.sell(coin_list[j],date_seq[i-1],0,para_min)
            if lock == 77:
                bad_sell_lock = 0
