...
if __name__ == '__main__':
    ...
    #现货API 1min
    response = hb.get_kline('eosusdt','1min',1000)
    resu = response['data']
    for i in range(1,len(resu)):
        state_dt = time.strftime("%Y-%m-%d-%H-%M", time.localtime(int(resu[i]['id'])))
        sql = "INSERT INTO eos_1min(state_dt,open_usd,close_usd,high_usd,low_usd,amount,vol,timestamp) VALUES ('%s','%.2f','%.2f','%.2f','%.2f','%.6f','%.4f','%s')"%(state_dt,float(resu[i]['open']),float(resu[i]['close']),float(resu[i]['high']),float(resu[i]['low']),float(resu[i]['vol']),float(resu[i]['amount']),str(resu[i]['id']))
        try:
            cursor.execute(sql)
            db.commit()
    ...
    # 现货API 5min
    response2 = hb.get_kline('eosusdt', '5min', 1000)
    resu2 = response2['data']
    for j in range(1, len(resu2)):
        state_dt2 = time.strftime("%Y-%m-%d-%H-%M", time.localtime(int(resu2[j]['id'])))
        sql2 = "INSERT INTO eos_5min(state_dt,open_usd,close_usd,high_usd,low_usd,amount,vol,timestamp) VALUES ('%s','%.2f','%.2f','%.2f','%.2f','%.6f','%.4f','%s')" % (
    ...
    # 现货API 15min
    response3 = hb.get_kline('eosusdt', '15min', 1000)
    resu3 = response3['data']
    for k in range(1, len(resu3)):
        state_dt3 = time.strftime("%Y-%m-%d-%H-%M", time.localtime(int(resu3[k]['id'])))
        sql3 = "INSERT INTO eos_15min(state_dt,open_usd,close_usd,high_usd,low_usd,amount,vol,timestamp) VALUES ('%s','%.2f','%.2f','%.2f','%.2f','%.6f','%.4f','%s')" % (
    ...
    # 现货API 30min
    response4 = hb.get_kline('eosusdt', '30min', 1000)
    resu4 = response4['data']
    for l in range(1, len(resu4)):
        state_dt4 = time.strftime("%Y-%m-%d-%H-%M", time.localtime(int(resu4[l]['id'])))
        sql4 = "INSERT INTO eos_30min(state_dt,open_usd,close_usd,high_usd,low_usd,amount,vol,timestamp) VALUES ('%s','%.2f','%.2f','%.2f','%.2f','%.6f','%.4f','%s')" % (
    ...
    # 现货API 60min
    response5 = hb.get_kline('eosusdt', '60min', 1000)
    resu5 = response5['data']
    for m in range(1, len(resu5)):
        state_dt5 = time.strftime("%Y-%m-%d-%H-%M", time.localtime(int(resu5[m]['id'])))
        sql5 = "INSERT INTO eos_60min(state_dt,open_usd,close_usd,high_usd,low_usd,amount,vol,timestamp) VALUES ('%s','%.2f','%.2f','%.2f','%.2f','%.6f','%.4f','%s')" % (
