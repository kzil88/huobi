...
def get_local_orders(coin,operation,new_price):
    ...
    for i in range(len(done_set)):
        if done_set[i][1]*1.002 < new_price:
...
if __name__ == '__main__':
    ...
    coin_list = ['btc', 'eos']
    interval = [150,0.34]
    para_list = [[150, -150], [0.68, -0.0]]

    db = pymysql.connect(host='127.0.0.1', user='root', passwd='admin', db='stock', charset='utf8')
    cursor = db.cursor()
    for a in range(len(coin_list)):
        sql_init = "select up_limit,low_limit from mix_acct a where a.coin = '%s' and up_limit is not null and low_limit is not null order by seq desc limit 1"%(coin_list[a])
        cursor.execute(sql_init)
        db.commit()
        done_init = cursor.fetchall()
        if len(done_init) > 0:
            para_list[a][0] = float(round(done_init[0][0]*100)/100)
            para_list[a][1] = float(round(done_init[0][1] * 100) / 100)
    cursor.close()
    db.close()
                        ...
                        while True:
                            try:
                                ...
                                if str(order_resp['status']) == 'ok':
                                    ...
                                    sql_sellorder = "insert into mix_acct(operation,coin,quant,price,amount,state_dt,order_id,src_code,up_limit,low_limit,total_cap,bz)values('%s','%s','%.4f','%.4f','%.2f','%s','%s','%s','%.2f','%.2f','%.2f','%s')" % (str('sell'), str(coin_list[i]), float(order_data['filled-amount']),float(order_data['price']),float(order_data['filled-amount']) * float(order_data['price']), order_state_dt,str(order_id), 'MIX_Bot4', float(para_list[i][0]), float(para_list[i][1]),float(round(new_cap * 100) / 100),str(order_data['type']))
                ...
                if divmod(singal_time_cnt, 4)[1] == 0:
                    ...
                    if time_delt >= 900:
                        ...
                        if resu_single < 0 and new_price[i] > base_price[i] * para_threshold:
                            ...
                            if len(seq_list) > 0:
                                cap_coin = math.floor(np.array(quant_list).sum() * 1000)/1000
                            else:
                                cap_coin = math.floor(cap_coin_trade[i] * 0.2 * 1000) / 1000
                                    ...
                                    while True:
                                        try:
                                            ...
                                            if str(order_resp['status']) == 'ok':
                                                ...
                                                order_ts = int(order_data['created-at'])
                                                order_state_dt = datetime.datetime.fromtimestamp(round(order_ts / 1000)).strftime("%Y-%m-%d-%H-%M-%S")
                                                sql_buyorder = "insert into mix_acct(operation,coin,quant,price,amount,state_dt,order_id,src_code,up_limit,low_limit,total_cap,bz)values('%s','%s','%.4f','%.4f','%.2f','%s','%s','%s','%.2f','%.2f','%.2f','%s')" % (str('buy'), str(coin_list[i]), float(order_data['filled-amount']),float(order_data['price']),float(order_data['filled-amount']) * float(order_data['price']),order_state_dt, str(order_id), 'MIX_Bot4', float(para_list[i][0]),float(para_list[i][1]), float(round(new_cap * 100) / 100),str(order_data['type']))
