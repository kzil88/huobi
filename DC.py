...
class data_collect2(object):
    ...
    def __init__(self,start_dt,end_dt,para_min,threshold):
        self.collectDATA(start_dt,end_dt,para_min,threshold)
    ...
    def collectDATA(self,start_dt,end_dt,para_min,threshold):
        ...
        cursor = db.cursor()
        if para_min == 'day':
            sql_done_set = "SELECT * FROM btc_day a where state_dt >= '%s' and state_dt <= '%s' order by state_dt asc" % (start_dt, end_dt)
        else:
            sql_done_set = "SELECT * FROM btc_%smin a where state_dt >= '%s' and state_dt <= '%s' order by state_dt asc" % (str(para_min), start_dt, end_dt)
