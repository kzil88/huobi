...
def Signal2(coin_symbol, para_min, para_up, para_low, show_len):
    ...
    macd_temp = ta.MACD(close, 12, 26, 9)
    action_flag1 = 0
    if (macd_temp[2][-2] < macd_temp[2][-1]) and (macd_temp[2][-2] < macd_temp[2][-3]) and (macd_temp[2][-1] < 0) and (macd_temp[2][-3] < 0) :
        action_flag1 = 1
    elif (macd_temp[2][-2] > macd_temp[2][-1]) and (macd_temp[2][-2] > macd_temp[2][-3]) and (macd_temp[2][-1] > 0) and (macd_temp[2][-3] > 0) :
        action_flag1 = -1
                        ...
                        if resu_single < 0 and new_price[i] > base_price[i] * para_threshold:
                            ...
                            if cap_coin >= min_deal_limit[i]:
                                ...
                                if str(resp['status']) == 'ok':
                                    para_list[i][0] = para_list[i][0] + interval
                                    para_list[i][1] = para_list[i][1] + interval
